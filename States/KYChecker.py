...
class Worker(multiprocessing.Process):
    ...
    def run(self):
        while True:
            ...
            if "OK" not in str(request.content):
                print(colored("PLATE UNAVAILABLE:  " + url[70:len(url)-52], 'red'))
            else:
                print(colored("PLATE AVAILABLE:  " + url[70:len(url)-52], 'green'))
                f = open("outputresults.txt", "a")
                f.write(url[70:len(url)-52] + "\n")
                f.close()
...
if __name__ == '__main__':
    ...
    index = 0
    stupidparse = ""
    urlfront = "https://secure2.kentucky.gov/kytc/plates/web/LicensePlate/Verify?text="
    urlback = "&licensePlateId=030af448-0201-471d-b561-d68376752ef6"
