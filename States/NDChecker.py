            ...
            while retries < self._max_retries:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        # "is not available" not being in the request's response means it is available.
                        # Record an available license plate combination.
                        if 'is not available.' not in response.text:
                            print(colored("PLATE AVAILABLE:  " + word, 'green'))
                            f = open("outputresults.txt", "a")
                            f.write(word + "\n")
                            f.close()

                        # "is not available" being in the request's response means it is taken.
