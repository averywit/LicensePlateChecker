            ...
            while retries < self._max_retries:
                try:
                    ...
                    response = session.get(
                        url=f"https://myrta.com/plateapi/plate/{word}?vehicleType=LIGHT_VEHICLE",
                        headers=headers,
                        timeout=10
                    )
                    ...
                    if response.status_code == 200 or response.status_code == 404:
                        ...
                        # '"available":true' not being in the request's response means it is taken.
                        # Do not record a taken license plate combination.
                        if '"available":true' not in response.text:
                            print(colored("PLATE UNAVAILABLE:  " + word, 'red'))

                        # '"available":true' being in the request's response means it is available.
